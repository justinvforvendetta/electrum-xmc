...
class Commands:
    ...
    @command('')
    def version(self):
        """Return the version of electrum-xmc."""
        import electrum_xmc  # Needs to stay here to prevent ciruclar imports
        return electrum_xmc.ELECTRUM_VERSION
    ...
    def setlabel(self, key, label):
        """Assign a label to an item. Item may be a xmc address or a
        transaction ID"""
    ...
    def _format_request(self, out):
        ...
        out['amount'] = format_satoshis(out.get('amount')) + ' xmc'
...
param_descriptions = {
    ...
    'destination': 'xmc address, contact or alias',
    'address': 'xmc address',
    'seed': 'Seed phrase',
    'txid': 'Transaction ID',
    'pos': 'Position',
    ...
    'amount': 'Amount to be sent (in xmc). Type \'!\' to send the maximum available.',
    'requested_amount': 'Requested amount (in xmc).',
    'csv_file': 'CSV file of recipient, amount',
}

command_options = {
    'broadcast':   (None, "--broadcast",   "Broadcast the transaction to the xmc network"),
    ...
    'tx_fee':      ("-f", "--fee",         "Transaction fee (in xmc)"),
...
config_variables = {
    ...
    'addrequest': {
        ...
        'url_rewrite': 'Parameters passed to str.replace(), in order to create the r= part of xmc: URIs. Example: \"(\'file:///var/www/\',\'https://electrum.org/\')\"',
    },
    'listrequests':{
        'url_rewrite': 'Parameters passed to str.replace(), in order to create the r= part of xmc: URIs. Example: \"(\'file:///var/www/\',\'https://electrum.org/\')\"',
    }
...
def get_parser(run_gui, run_daemon, run_cmdline):
    ...
    parser_gui.add_argument("url", nargs='?', default=None, help="xmc URI (or bip70 file)")
