...
from electrum_xmc import Interface, SimpleConfig
from electrum_xmc.network import filter_protocol, parse_servers

# electrum_xmc.util.set_verbosity(1)

def get_peers():
    # 1. start interface and wait for connection
    q = Queue.Queue()
    interface = electrum_xmc.Interface('ecdsa.net:110:s', q)
    interface.start()
    i, r = q.get()
