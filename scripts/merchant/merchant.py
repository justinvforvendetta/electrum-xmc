...
import electrum
electrum_xmc.set_verbosity(False)
...
if __name__ == '__main__':
    ...
    # start network
    c = electrum_xmc.SimpleConfig({'wallet_path':wallet_path})
    daemon_socket = electrum_xmc.daemon.get_daemon(c, True)
    network = electrum_xmc.NetworkProxy(daemon_socket, config)
    network.start()
    ...
    storage = electrum_xmc.WalletStorage(c)
    if not storage.file_exists:
        print "creating wallet file"
        wallet = electrum_xmc.wallet.Wallet.from_xpub(xpub, storage)
    else:
        wallet = electrum_xmc.wallet.Wallet(storage)
