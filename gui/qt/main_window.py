...
from electrum_xmc.bitcoin import MIN_RELAY_TX_FEE, COIN, is_valid
from electrum_xmc.plugins import run_hook
from electrum_xmc.i18n import _
from electrum_xmc.util import block_explorer, block_explorer_info, block_explorer_URL
from electrum_xmc.util import print_error, print_msg
from electrum_xmc.util import format_satoshis, format_satoshis_plain, format_time, NotEnoughFunds, StoreDict
from electrum_xmc import Transaction
from electrum_xmc import mnemonic
from electrum_xmc import util, bitcoin, commands, Wallet
from electrum_xmc import SimpleConfig, Wallet, WalletStorage
from electrum_xmc import Imported_Wallet
from electrum_xmc import paymentrequest
from electrum_xmc.contacts import Contacts
...
from electrum_xmc import ELECTRUM_VERSION
import re
...
from electrum_xmc.paymentrequest import PR_UNPAID, PR_PAID, PR_UNKNOWN, PR_EXPIRED
from electrum_xmc.paymentrequest import PaymentRequest, InvoiceStore, get_payment_request
...
class ElectrumWindow(QMainWindow):
    ...
    def __init__(self, config, network, gui_object):
        ...
        self.setWindowIcon(QIcon(":icons/electrum_xmc.png"))
        self.init_menubar()
    ...
    def load_wallet(self, wallet):
        ...
        title = 'Electrum-XMC %s  -  %s' % (self.wallet.electrum_version, self.wallet.basename())
    ...
    def backup_wallet(self):
        ...
        if new_path != path:
            try:
                ...
                QMessageBox.critical(None,"Unable to create backup", _("Electrum-XMC was unable to copy your wallet file to the specified location.")+"\n" + str(reason))
    ...
    def init_menubar(self):
        ...
        tools_menu.addAction(_("Electrum-XMC preferences") if sys.platform == 'darwin' else _("Preferences"), self.settings_dialog)
    ...
    def show_about(self):
        QMessageBox.about(self, "Electrum-XMC",
            _("Version")+" %s" % (self.wallet.electrum_version) + "\n\n" + _("Electrum-XMC's focus is speed, with low resource usage and simplifying xmc. You do not need to perform regular backups, because your wallet can be recovered from a secret phrase that you can memorize or write on paper. Startup times are instant because it operates in conjunction with high-performance servers that handle the most complicated parts of the xmc system."))

    def show_report_bug(self):
        QMessageBox.information(self, "Electrum-XMC - " + _("Reporting Bugs"),
    ...
    def notify(self, message):
        if self.tray:
            self.tray.showMessage("Electrum-XMC", message, QSystemTrayIcon.Information, 20000)
    ...
    def create_receive_tab(self):
        ...
        msg = _('xmc address where the payment should be received. Note that each payment request uses a different xmc address.')
        ...
        msg = ' '.join([
            ...
            _('Expired requests have to be deleted manually from your list, in order to free the corresponding xmc addresses'),
        ])
    ...
    def create_send_tab(self):
        ...
        msg = _('Recipient of the funds.') + '\n\n'\
              + _('You may enter a xmc address, a label from your list of contacts (a list of completions will be proposed), or an alias (email-like address that forwards to a xmc address)')
        ...
        msg = _('xmc transactions are in general not free. A transaction fee is paid by the sender of the funds.') + '\n\n'\
    ...
    def read_send_tab(self):
        ...
        for _type, addr, amount in outputs:
            if addr is None:
                QMessageBox.warning(self, _('Error'), _('xmc Address is None'), _('OK'))
                return
            if _type == 'address' and not bitcoin.is_address(addr):
                QMessageBox.warning(self, _('Error'), _('Invalid xmc Address'), _('OK'))
                return
            if amount is None:
    ...
    def pay_to_URI(self, URI):
        ...
        try:
            ...
            QMessageBox.warning(self, _('Error'), _('Invalid xmc URI:') + '\n' + str(e), _('OK'))
            return
        self.tabs.setCurrentIndex(1)
    ...
    def new_account_dialog(self, password):
        ...
        e = QLineEdit()
        vbox.addWidget(e)
        msg = _("Note: Newly created accounts are 'pending' until they receive xmc.") + " " \
    ...
    def tx_from_text(self, txt):
        ...
        if is_hex:
            try:
                ...
                traceback.print_exc(file=sys.stdout)
                QMessageBox.critical(None, _("Unable to parse transaction"), _("Electrum-XMC was unable to parse your transaction"))
                return
    ...
    def read_tx_from_qrcode(self):
        from electrum_xmc import qrscanner
        try:
        ...
        if not data:
            return
        # if the user scanned a xmc URI
        if data.startswith("xmc:"):
    ...
    def read_tx_from_file(self):
        ...
        try:
            ...
            QMessageBox.critical(None, _("Unable to read file or no transaction found"), _("Electrum-XMC was unable to open your transaction file") + "\n" + str(reason))
    ...
    def export_privkeys_dialog(self, password):
        ...
        defaultname = 'electrum-xmc-private-keys.csv'
        ...
        try:
            ...
            export_error_label = _("Electrum-XMC was unable to produce a private key-export.")
    ...
    def do_import_labels(self):
        ...
        try:
            ...
            QMessageBox.critical(None, _("Unable to import labels"), _("Electrum-XMC was unable to import your labels.")+"\n" + str(reason))


    def do_export_labels(self):
        labels = self.wallet.labels
        try:
            fileName = self.getSaveFileName(_("Select file to save your labels"), 'electrum_xmc_labels.dat', "*.dat")
            if fileName:
                with open(fileName, 'w+') as f:
                    json.dump(labels, f)
                QMessageBox.information(None, _("Labels exported"), _("Your labels where exported to")+" '%s'" % str(fileName))
        except (IOError, os.error), reason:
            QMessageBox.critical(None, _("Unable to export labels"), _("Electrum-XMC was unable to export your labels.")+"\n" + str(reason))
    ...
    def export_history_dialog(self):
        ...
        vbox = QVBoxLayout(d)
        defaultname = os.path.expanduser('~/electrum-xmc-history.csv')
        ...
        try:
            ...
            export_error_label = _("Electrum-XMC was unable to produce a transaction export.")
    ...
    def settings_dialog(self):
        ...
        from electrum_xmc.i18n import languages
        ...
        def on_unit(x):
            ...
            elif unit_result == 'mxmc':
                self.decimal_point = 5
            elif unit_result == 'uxmc':
        ...
        if self.need_restart:
            QMessageBox.warning(self, _('Success'), _('Please restart Electrum-XMC to activate the new GUI settings'), _('OK'))



    def run_network_dialog(self):
        if not self.network:
            QMessageBox.warning(self, _('Offline'), _('You are using Electrum-XMC in offline mode.\nRestart Electrum-XMC if you want to get connected.'), _('OK'))
            return
    ...
    def plugins_dialog(self):
        from electrum_xmc.plugins import plugins, descriptions, is_available, loader

        self.pluginsdialog = d = QDialog(self)
        d.setWindowTitle(_('Electrum-XMC Plugins'))
        d.setModal(1)
