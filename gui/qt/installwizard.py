...
from electrum_xmc import Wallet
from electrum_xmc import bitcoin
from electrum_xmc import util
...
from electrum_xmc.plugins import always_hook, run_hook
from electrum_xmc.mnemonic import prepare_seed

MSG_ENTER_ANYTHING    = _("Please enter a seed phrase, a master key, a list of xmc addresses, or a list of private keys")
...
class InstallWizard(QDialog):
    ...
    def __init__(self, config, network, storage, app):
        ...
        self.setWindowTitle('Electrum-XMC' + '  -  ' + _('Install Wizard'))
    ...
    def restore_or_create(self):
        vbox = QVBoxLayout()
        main_label = QLabel(_("Electrum-XMC could not find an existing wallet."))
        ...
        for i, (wtype,name) in enumerate(self.wallet_types):
            if not filter(lambda x:x[0]==wtype, electrum_xmc.wallet.wallet_types):
                continue
    ...
    def waiting_dialog(self, task, msg= _("Electrum-XMC is generating your addresses, please wait.")):
        def target():
            task()
    ...
    def network_dialog(self):
        ...
        label = QLabel(_("Electrum-XMC communicates with remote servers to get information about your transactions and addresses. The servers all fulfil the same purpose only differing in hardware. In most cases you simply want to let Electrum-XMC pick one at random if you have a preference though feel free to select a server manually.") + "\n\n" \
    ...
    def run(self, action, wallet_type):
        ...
        if action in ['create', 'restore']:
            if wallet_type == 'multisig':
                ...
                hardware_wallets = map(lambda x:(x[1],x[2]), filter(lambda x:x[0]=='hardware', electrum_xmc.wallet.wallet_types))
